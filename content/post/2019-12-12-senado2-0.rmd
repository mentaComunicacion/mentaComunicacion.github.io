...
(*Análisis del equipo de datos de mentaComunicación*[^1]) 

[^1]: Camila Higa, Franco Galeano, Carolina Rossi y Juan Pablo Ruiz Nicolini.
...
En el caso del Senado, hay 9 los legisladores ausentes en la plataforma, 8 de los cuales pertenecen al oficialismo y 1 a la oposición. Los ausentes de oficialismo son Ledesma Abdala de Zamora, Montenegro y Neder (Santiago del Estero), González y Mayans (Formosa), García Larraburu (R.Negro), Gimenez (Salta) y López (S.Juan);y por la oposición Olalla de Moreira (E.Ríos). 


**Qué sabemos de las $63$ cuentas de senadores que están presentes en Twitter?**

1. Que si agregamos todos sus "amigos" (*follows*) el total asciende a más de $41$ mil cuentas. Pero al contar usuarios únicos el número se reduce a poco más de $26$ mil. 

2. Entonces, el promedio de usuarios seguidos es de unas $413$ cuentas por `@` de senadores. Pero la realidad marca que existen grandes diferencias entre las mismas. Por ejemplo, mientras que la cuenta `reutemannc` sigue a solo $7$ usuarios, `lauramachado` lo hace con más de $3_{mil}$ usuarios. 

3. Por otro lado, podemos analizar las cuentas individualmente evaluando algunas características de su comportamiento: **1.** la frecuencia de publicación, **2.** la cantidad de interacciones promedio que sus publicaciones generan y **3.** su alcance potencial (medido por la cantidad de cuentas que los siguen - _followers_ ). 
...
Definimos el alcance potencial de cada cuenta en función de la cantidad de seguidores que cada una tiene. El gráfico siguiente muestra las $10/63$ cuentas con mayor cantidad de seguidores. Solo las cuenta de Martín Lousteau (con XXX seguidores) y Esteban Bullrich (con XXXX) superan la barrera de $450_{mil}$ seguidores. Más parejos, cerca de los $150_{mil}$ se ubican los perfiles de Mariano Recalde y Julio Cobos. Dentro del *top10* las cuentas de Fernández Sagasti y Alfredo de Angeli rondan los XXX seguidores. 
...
Pero, además de considerar la cuenta absoluta, también puede evaluarse en relación a la cantidad de cuentas que ellos mismos siguen (construyendo un ratio $seguidores/seguidos$). En el gráfico esto está representado por el tamaño del círculo: cuanto más grande es, mayor el valor del ratio. 
En la cola del *ranking* de más seguidos, $8/10$ cuentas no superan la barrera de los $1500$ seguidores. 
...
Otro comportamiento que podemos medir se focaliza en el nivel de actividad. Es decir, en la frecuencia con la que las cuentas de los senadores publican contenido orgánico. La primera conclusión es que los miembros de la cámara alta son relativamente moderados: solo $6$ de las $63$ cuentas relevadas superan las $3$ publicaciones diarias en promedio.  
...
Consideramos para este indicador el promedio de la cantidad de **Me Gusta** y **Retweet** que recibe una publicación determinada, habiendo restringido la muestra de *tuits* a todas las publicaciones de senadores durante el año $2019$. 

Solo $3$ cuentas superan las $500$ interacciones promedio por publicación: Recalde, Naidenoff y Bullrich. Resulta llamativa la diferencia entre la cuenta de Bullrich respecto de sus colegas. 
...
1. Cada círculo correspode a una cuenta de una senadora o senador.
2. Las flechas (*aristas*) marca la dirección de la conexión. Esto es, quién *sigue* a quién.
...
Previsiblemente se visualiza una mayor intesidad de conexiones (aristas) entre miembros de cada uno de los espacios políticos: mayor cantidad de conexiones azules entre miembros del Frente de Todos y mayor cantidad de conexiones amarillas entre los de Juntos por el Cambio. 


[Una versión interactiva ](https://mentacomunicacion.github.io/interactivos/sen_igraph.html) nos permite seleccionar a los distintos usuarios (*nodos*) para visualizar la red que conforman sus relaciones individuales. Tomemos como ejemplo los vínculos que se generan con la cuenta `CarlosMenem_LR`  como _nodo_ de referencia. 
...
Más allá del vínculo específico entre miembros del congreso, la información recolectada nos permite avanzar sobre la totalidad de las cuentas. Como veremos a continuación, al analizar el listado de *seguidos/amigos* de cada legislador podemos hechar luz sobre sus posicionamientos políticos por fuera de los límites de la Cámara. Sabemos que la suma de senadores alcanza unas $26_{mil}$ cuentas distintas. Pero, ¿cuáles son más representativas entre los seguidos por senadores? ¿Qué diferencias se pueden encontrar según el alineamiento político?


**Cuentas más seguidas** 

El gráfico que sigue muestra las 30 cuentas más seguidas por senadores argentinos. Entre ellas se destacan cuentas insticuionales (`@SenadoArgentina`, `@AgenciaTelam`, `@elparlamentario`), de medios (`@LANACION`, `@infobae`, `@clarincom`, `@AgenciaTelam`, `@todonoticias`, `@LPOArg`), de periodistas (`@Gatosylvestre`, `@luisnovaresio`, `@VHMok`) y dirigentes políticos (`@mauriciomacri`, `@CFKArgentina`, `@SergioMassa`, `@alferdez`), entre otros.
...
Existen cuentas como `SenadoArgentina` o `perfilcom` que son seguidas por una cantidad equivalente de legisladores de ambos espacios. En cambio, para otras como `mauriciomacri` y `alferdez` se aprecia un desbalance en función de sus pertenencias políticas.  

Si calculamos la distancia entre espacios para las cuentas con más seguidores, se evidencian estos sesgos partidarios. Hacia la derecha, cuando son mayoritariamente oficialistas. Y, a la inversa, hacia la izquierda cuando son mayoritariamente oposición. Además, aunque la mayoría de los medios de comunicación se encuentran más cercanos a una *zona de empate*, las cuentas `radiomitre` -`todonoticias` y `VHMok` - `C5N` se alinean más a cada uno de los espacios políticos. 
...
Para profundizar al respecto nos propusimos en primer lugar replicar el análisis de [Michael Kearney]( https://twitter.com/kearneymw/status/1187362531859259392?s=09) sobre el 116º Congreso de Estados Unidos analizando las cuentas de les Senadores de Argetnina. 
...
Un ejercicio alternativo fue poner el foco en la cuentas más seguidas para cada espacio político. Para ello
restringimos el *ranking* luego de ordenar los datos minimizando la cantidad de seguidores de cada uno de los espacios. Estas cuentas se encuentran más pegadas a los ejes respectivos de cada ecosistema en el gráfico anterior. Las tablas que siguen **rankean** estas cuentas bajo $2$ condiciones ordenando el ranking según prevalencia de seguidores (i) oficialistas y (ii) opositores.
...
Siguiendo la idea de *Calvo y Malamud* identificamos todas las cuentas seguidas por cada uno de los legisladores. Con esa información construimos una matriz en la que cada fila se corresponde con un legislador y cada columna con cada uno de estos "amigos". Las celdas, en tanto, identifican con un $1$ cuando la cuenta de un legislador sigue a una cuenta y con un $0$ cuando no lo hace. 

El paso siguiente es calcular cómo correlaciona la lista de `seguidos` de cada una de las cuentas de legisladores comparada con sus pares. Visualmente puede analizarse de la siguiente manera: 
...
Los ejes listan (agrupadas por espacio político) cada una de las cuentas analizadas para el Senado de la Nación. La diagonal inferior (azul oscuro) muestra la correlación política perfecta ($1$) dado que las observaciones corresponden a la misma cuenta en filas y columnas. Así, para el resto de las cuentas con las que se compara cada usuario, cuanto más azul, la correlación será más alta (los usuarios que siguen las cuentas son más similares) y cuanto más rojo, los "amigos" de ese par de cuentas difieren más. 

Rápidamente se observa que en líneas generales hay una asociación positiva entre cuentas del Frente de Todos (el oficialismo, en azul) y de la oposición (agrupados con distintos colores por pertenencia de bloque dentro del interbloque Juntos por el Cambio). Por último, al comparar entre cuentas pertenecientes a polos distintos la tendencia es a la inexistencia de relación (más blanco, con valores cercanos a $0$; o una asocación negativa, con tonos más rojos). 

El siguiente gráfico ubica en un plano a los dos primeros [componentes principales](https://es.wikipedia.org/wiki/An%C3%A1lisis_de_componentes_principales) (que explican la mayor variabilidad de los datos de la matriz de usuarios seguidos por senadores). El gráfico ordena los `@` de cada uno en distintos espacios y destaca la pertenencia política de cada uno. 
...
Un último ejercicio consitió en revisar **cómo interactúa el público** con las publicaciones emitidas desde las cuentas de senadores.  Como ya desarrollamos en una [entrada previa](https://mentacomunicacion.github.io/2019/11/13/interacciones-en-twitter/), les usuaries interactúan con el contenido de tres formas [^2]: 
...
***"Si tuviste más respuestas a a un tuit que favs o RT, entonces la cagaste"***. Cuando el `RATIO` (originalmente descripto [acá](https://www.esquire.com/news-politics/news/a54440/twitter-ratio-reply/)) toma valores altos es indicador de que la recepción del tuit fue muy mala. 
