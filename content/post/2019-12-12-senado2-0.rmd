...
2. El promedio Entonces, el promedio de usuarios seguidos es de unas $413$ cuentas por `@` de senadores ($663$ es el valor tomando el promedio simple, sin discriminar repeticiones). Pero la realidad marca que existen grandes diferencias entre las mismas. Por ejemplo, mientras que la cuenta `reutemannc` sigue a solo $7$ usuarios, `lauramachado` lo hace con más de $3_{mil}$ usuarios. 
...
El gráfico que sigue resume estos tres indicadores para las cunetas de seandores relevadas con la excepción de `reutemannc` y `mariabtapia` que no registraron actividad en $2019$. Además de las observaciones de cada cuenta los gráficos de cajas muestran la distribución de los mismos. La línea horizontal, por caso, representa la mediana de cada uno de los indicadores. Estos son los valores que dividen a las muestras en dos mitades: 

* Frecuencia promedio: $0.7$ tuiuts por día
...
Otro comportamiento que podemos medir se focaliza en el nivel de actividad. Es decir, en la frecuencia con la que las cuentas de los senadores publican contenido orgánico. La primera conclusión es que los miembros de la cámara alta son relativamente moderados: solo $6$ de las $63$ cuentas relevadas superan las $3$ publicaciones diarias en promedio, tal como muestra el *ranking* de las $10$ cuentas que publcian con mayor periodicidad. 
...
1. Cada círculo correspode a una cuenta de una senadora o senador (*nodo*).
...
Previsiblemente se visualiza una mayor intesidad de conexiones entre miembros de cada uno de los espacios políticos: mayor cantidad de conexiones azules entre miembros del Frente de Todos y mayor cantidad de conexiones amarillas entre los de la oposición. 
...
* `pameverasay` en la cuenta que más sigue a senadores de la oposición ($30$), seguida por `laurmachado` ($14$)
* La cuenta con más seguida por sus pares es `estabanbullrich` ($28$) y la menos seguida es `vzimmermannok` ($1$)
...
## Ecosistemas políticos en Twitter
  ...
  **Cuentas más seguidas** 
  ...
  El gráfico que sigue muestra las 30 cuentas más seguidas por senadores argentinos. Entre ellas se destacan cuentas insticuionales (`@SenadoArgentina`, `@AgenciaTelam`, `@elparlamentario`); de medios (`@LANACION`, `@infobae`, `@clarincom`, `@AgenciaTelam`, `@todonoticias`, `@LPOArg`); de periodistas (`@Gatosylvestre`, `@luisnovaresio`, `@VHMok`) y de dirigentes políticos (`@mauriciomacri`, `@CFKArgentina`, `@SergioMassa`, `@alferdez`), entre otros.
...
Si calculamos la distancia entre espacios para las cuentas con más seguidores, se evidencian estos diferencias partidarias. Hacia la derecha, cuando son mayoritariamente oficialistas. Y, a la inversa, hacia la izquierda cuando son mayoritariamente oposición. Además, aunque la mayoría de los medios de comunicación se encuentran más cercanos a una *zona de empate*, las cuentas `radiomitre` -`todonoticias` y `VHMok` - `C5N` se alinean más a cada uno de los espacios políticos. 
