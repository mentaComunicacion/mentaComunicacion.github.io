...
Rápidamente se observa que en líneas generales hay una asociación positiva entre cuentas del Frente de Todos (el oficialismo, con las etiquetas en azul) y de la oposición (agrupadas con distintos colores por pertenencia de bloque dentro del interbloque Juntos por el Cambio). 

En la parte superior (correspondiente a los perfiles clasificados como Frente de Todos), sin embagro, se aprecian observaciones con correlación negativa (en rojo) en cuentas como `CarlosMenem_LR` y `juanmariopais`. 

Por último, al comparar entre cuentas pertenecientes a polos distintos la tendencia es a la inexistencia de relación (más blanco, con valores cercanos a $0$; o una asocación negativa, con tonos más rojos). 

---

El siguiente gráfico ubica en un plano a los dos primeros [componentes principales](https://es.wikipedia.org/wiki/An%C3%A1lisis_de_componentes_principales) (que explican la mayor variabilidad de los datos de la matriz de usuarios seguidos por senadores)[^2]. El gráfico ordena los `@` de cada uno en distintos espacios y destaca la pertenencia política de cada uno.
...
Un último ejercicio consitió en revisar **cómo interactúa el público** con las publicaciones emitidas desde las cuentas de senadores.  Como ya desarrollamos en una [entrada previa](https://mentacomunicacion.github.io/2019/11/13/interacciones-en-twitter/), les usuaries interactúan con el contenido de tres formas[^3]: 
